...
test_current_routing() {
    ...
    response=$(curl -s -w "%{http_code}" "https://league-creator-1.preview.emergentagent.com/socket.io/?EIO=4&transport=polling" -o /tmp/socketio_response.txt)
...
verify_fix() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if test_current_routing; then
            ...
            api_response=$(curl -s -w "%{http_code}" "https://league-creator-1.preview.emergentagent.com/api/health" -o /dev/null)
            ...
            frontend_response=$(curl -s -w "%{http_code}" "https://league-creator-1.preview.emergentagent.com/" -o /dev/null)
...
show_manual_instructions() {
    ...
    echo "   curl 'https://league-creator-1.preview.emergentagent.com/socket.io/?EIO=4&transport=polling'"
