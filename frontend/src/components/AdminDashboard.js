            ...
            <Card>
              ...
              <CardContent className="space-y-6" data-testid={TESTIDS.settingsForm}>
                ...
                <div className="flex justify-end">
                  <Button onClick={handleSettingsUpdate} disabled={saving} data-testid={`${TESTIDS.settingsSave} ${TESTIDS.submitButton}`}>
                    {saving ? (
                      <>
                    ...
                    {league.status === 'ready' && (
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            {members.length < (leagueSettings?.leagueSize?.min || league.settings.league_size?.min || 2) 
                              ? `Need at least ${leagueSettings?.leagueSize?.min || league.settings.league_size?.min || 2} managers to start auction (currently have ${members.length})`
