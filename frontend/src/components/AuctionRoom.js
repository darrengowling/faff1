...
import { createSocket, getApiOrigin } from '../lib/socket';
...
const AuctionRoom = ({ user, token }) => {
  ...
  // Unified WebSocket connection state
  const [socket, setSocket] = useState(null);
  const [joinAndSync, setJoinAndSync] = useState(null);
  ...
  // Set up auction-specific event handlers on the unified socket
  const setupAuctionEventHandlers = (socket) => {
    ...
    socket.on('auction_snapshot', (snapshot) => {
    ...
    // Heartbeat system for server time sync
  ...
  // Legacy connection management functions removed - now using unified socket factory
