      ...
      <header className="fixed top-0 w-full bg-white/95 backdrop-blur-sm border-b z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <BrandBadge variant="full" size="md" onClick={() => scrollToSection('home')} className="cursor-pointer" />
              ...
              <div className="px-2 pt-2 pb-3 space-y-1">
                {navSections.slice(1).map((section) => (
                  <button
                    key={section.id}
                    onClick={() => scrollToSection(section.id)}
                    className="block px-3 py-2 text-base font-medium rounded-md w-full text-left transition-colors text-gray-700 hover:text-gray-900 hover:bg-gray-50"
                  >
                    {section.label}
                  </button>
                ))}
