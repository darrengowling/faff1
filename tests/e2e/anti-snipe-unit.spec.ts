...
test.describe('Anti-Snipe Logic Unit Tests', () => {
  test('Backend anti-snipe logic uses deterministic time', async ({ page }) => {
    ...
    const response = await page.request.post('https://pifa-stability.preview.emergentagent.com/api/test/time/advance', {
      data: { ms: 1000 }
    });
  ...
  test('Time advancement maintains consistent intervals', async ({ page }) => {
    ...
    await page.request.post('https://pifa-stability.preview.emergentagent.com/api/test/time/set', {
  ...
  test('Socket reconnect uses controlled backoff in test mode', async ({ page }) => {
    ...
    await page.goto('https://pifa-stability.preview.emergentagent.com/diagnostics');
  ...
  test('Anti-snipe threshold configuration from environment', async ({ page }) => {
    ...
    const healthResponse = await page.request.get('https://pifa-stability.preview.emergentagent.com/api/health');
    ...
    const currentTime = await page.request.post('https://pifa-stability.preview.emergentagent.com/api/test/time/advance', {
      data: { ms: 0 }
    });
