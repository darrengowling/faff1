...
if TEST_MODE:
    ...
    cors_origins.extend(["http://localhost:3000", "https://leaguemate-1.preview.emergentagent.com"])
    # Remove duplicates
...
# Custom middleware to handle /api/socket.io/diag before Socket.IO intercepts it
from starlette.middleware.base import BaseHTTPMiddleware
from starlette.responses import JSONResponse

class SocketIODiagMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        # Intercept /api/socket.io/diag requests before they reach Socket.IO
        if request.url.path == "/api/socket.io/diag":
...
async def request_magic_link(request: MagicLinkRequest):
    ...
    try:
        ...
        if is_development or TEST_MODE:
            frontend_url = os.getenv("FRONTEND_URL", "https://leaguemate-1.preview.emergentagent.com")
...
if TEST_MODE:
    ...
    async def check_league_ready(league_id: str):
        """
        TEST_MODE probe: GET /test/league/:id/ready → { ready: true } 
        when league, settings, and ≥1 membership exist.
        """
        if not is_test_mode():
            raise HTTPException(
        ...
        try:
            # Check if league exists and has settings
            league = await db.leagues.find_one({"_id": league_id})
            if not league:
                return {"ready": False, "reason": "league_not_found"}
            
            # Verify league has settings
            if not league.get("settings"):
                return {"ready": False, "reason": "league_settings_missing"}
            
            # Check if at least 1 membership exists
            membership_count = await db.league_memberships.count_documents({
                "league_id": league_id
            })
            if membership_count == 0:
                return {"ready": False, "reason": "no_memberships_found"}
            
            # All atomic requirements met: league exists, has settings, and ≥1 membership
            logger.info(f"🧪 LEAGUE READY CHECK: League {league_id} ready with {membership_count} memberships")
            return {"ready": True}
...
async def join_league_direct(
    ...
    # Emit real-time event to league room
    ...
    await sio.emit('league_status_update', {
        'league_id': league_id,
...
# Create Socket.IO sub-application and mount it at /api/socket.io
socketio_sub_app = socketio.ASGIApp(sio)
fastapi_app.mount("/api/socket.io", socketio_sub_app)
